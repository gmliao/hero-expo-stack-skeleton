...
- **Backend MVC layers (required):** The backend follows a strict three-layer architecture. Each layer has a single responsibility and must only call the layer directly below it:
  - **Endpoint (Controller)** — `src/controllers/<domain>/`: HTTP input/output only. Extract validated input, call the Service method, return the result. Must NOT import or reference any Repository interface or implementation. No business logic, no `findById`, no uid checks here.
...
- **Deps.services namespace (required):** All domain services are injected under `Deps.services` (e.g. `deps.services.todos`). Infrastructure (`auth: IAuthVerifier`, `logger: Logger`) stays at the top level of `Deps`. This allows cross-service access inside `execute()` via `deps.services.<domain>.*` without coupling Controller constructors to service instances. When adding a new domain service, add it to `DepsServices` in `src/http/endpoint.ts` and wire it in `createDeps()` in `src/deps.ts`.
